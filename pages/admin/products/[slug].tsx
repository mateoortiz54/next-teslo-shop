...
const ProductAdminPage:FC<Props> = ({ product }) => {
    ...
    useEffect(() => {
      const subscription = watch((values, {name, type}) => {
            if (name === 'title') {
                const newValue = values.title?.trim()
                                .replaceAll(' ', '_')
                                .replaceAll('´', '')
                                .toLocaleLowerCase() || '';
    ...
    const onNewTag = (tag:string) => {
        ...
        if (!tags.includes(newTag) && tag !== '' && tag !== ' '  ) {
            tags.push(newTag)
            setValue('tags', tags, {shouldValidate: true})
            setNewValueTag('')
        }
        setNewValueTag(t => t.replace(' ', ''))
    ...
    const onFileSelected = async({target}: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            for (const file of target.files){
                ...
                const {data} = await tesloApi.post<{message:string}>('/admin/upload', formDataFiles)
                // console.log(data)
    ...
    const onSubmitForm = async(formData: FormData) => {
        ...
        try {
            const {data} = await tesloApi({
                url: '/admin/products',
                method: formData._id ? 'PUT': 'POST', // si tenemos un  _id, entonces acutalizar, sino crear
                data: formData
            })
            ...
            if (!formData._id) {
                ...
                setIsSaving(false)
                //Para que pueda seguir editando
            }
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={ 6 }>
                        ...
                        <TextField
                            ...
                            variant="filled"
                            fullWidth 
                            multiline={false} // En true entra en bucle
                            sx={{ mb: 1 }}
                    ...
                    <Grid item xs={12} sm={ 6 }>
                        ...
                        <TextField
                            ...
                            value={newValueTag}
                            onChange={({target})=> setNewValueTag(target.value) }
                            onKeyPress={(event: any) => {
                                if (event.key === ' ' )
                                    
                                    onNewTag(event.target.value );
